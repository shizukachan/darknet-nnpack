...
void play_detector(char *datacfg, char *cfgfile, char *weightfile, char *path, float thresh, float hier_thresh)
{
	list *options = read_data_cfg(datacfg);
	char *name_list = option_find_str(options, "names", "data/names.list");
	char **names = get_labels(name_list);
...
void run_detector(int argc, char **argv)
{
	...
	else if(0==strcmp(argv[2], "play")) play_detector(datacfg, cfg, weights, filename, thresh, hier_thresh);
